...
{
	public static function homepage($attributes, $content = null)
	{
	    ...
	    $week = Review::select('COUNT(*) AS c, YEAR(created_at) AS year, WEEK(created_at) AS week,created_at')
	    ...
	    return View::render('homepage', compact('total', 'months', 'average', 'reviews'));
	}		
	...
	public static function average($attributes, $content = null)
	{
	    ...
	    return View::render('average', compact('total', 'months', 'average'));
	}
	...
	public static function slider($attributes, $content = null)
		{
			...
			return View::render('slider', compact('reviews'));
		}
	...
	public static function showForm($attributes, $content = null)
	{
		...
		return @View::render('form', compact('a'));
	}
	...
	{   
		...
		$mailarray = array('5','4'); 				$selectOption=get_option('reviews-mailchimp-settings');				$apiKey = $selectOption['mailchimpApikey'];
		//if( in_array(trim($_POST['rating']),$mailarray) && $_POST['recommend'] == 'Yes' ){	
		//echo $test=Functions::InsertdataMailChimp($data);						
		//}
		if(!empty($apiKey)){		echo $test=Functions::InsertdataMailChimp($data);			}
	...
	public static function sendToHeadOffice(Review $review)
	{
		...
		$mailResult = false;
		$mailResult = wp_mail( $emails, $subject, View::render('mail.head', compact('review', 'hide_approval_links')), $headers);
